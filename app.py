...
import analytics_routes as analytics_routes_mod
...
analytics_routes_mod.register(app)
...
def create_amazon_ads_account() -> Any:
    ...
    Body: {
        ...
        "env": "production",
        "lwa_client_id": "amzn1.application-oa2-client...",   # optional, overrides .env
        "lwa_client_secret": "...",                            # optional, overrides .env
        "notes": ""
    }
    """
    ...
    try:
        acct_id = bookdb.save_amazon_ads_account(
            ...
            client_id=(body.get("lwa_client_id") or "").strip(),
            lwa_client_secret=(body.get("lwa_client_secret") or "").strip(),
    ...
    try:
        ...
        return jsonify({
            "warning": (
                "Account saved but token validation failed: "
                f"{exc}. Check your credentials in Settings → Amazon Ads."
            ),
            "id": acct_id,
        }), 200
...
@app.patch("/api/amazon-ads/accounts/<account_id>")
def update_amazon_ads_account(account_id: str) -> Any:
    """Update credentials or label of an existing account.

    Body fields are all optional — only non-empty values are applied.
    Send empty string for lwa_client_id/lwa_client_secret to clear them
    (reverting to the .env global defaults).
    """
    acct = bookdb.get_amazon_ads_account(account_id, include_token=True)
    if not acct:
        return jsonify({"error": "account not found"}), 404
    body = request.get_json(silent=True) or {}

    label = body.get("label", acct["label"]).strip() or acct["label"]
    token = body.get("lwa_refresh_token", acct["lwa_refresh_token"]).strip() or acct["lwa_refresh_token"]
    env = body.get("env", acct.get("env", "production")).strip() or "production"
    # Allow clearing per-account credentials by sending empty string explicitly
    client_id = body["lwa_client_id"].strip() if "lwa_client_id" in body else (acct.get("client_id") or "")
    client_secret = body["lwa_client_secret"].strip() if "lwa_client_secret" in body else (acct.get("lwa_client_secret") or "")
    notes = body.get("notes", acct.get("notes", ""))

    try:
        bookdb.save_amazon_ads_account(
...
@app.get("/api/amazon-ads/app-config")
def amazon_ads_app_config() -> Any:
    """Return whether the shared LWA app credentials are set in .env (no secrets exposed)."""
    import os
    client_id = bool(os.getenv("LWA_CLIENT_ID", "").strip())
    client_secret = bool(os.getenv("LWA_CLIENT_SECRET", "").strip())
    redirect_uri = os.getenv("AMAZON_ADS_OAUTH_REDIRECT_URI", "").strip()
    return jsonify({
...
# ---------------------------------------------------------------------------
# Amazon Ads — Browser-based OAuth 2.0 login flow
# ---------------------------------------------------------------------------
import os as _os
import secrets as _secrets
import time as _oauth_time

_oauth_states: dict[str, float] = {}   # state -> expires_at epoch
_oauth_pending: dict[str, dict] = {}   # tmp_id -> {refresh_token, profiles, expires_at}


def _clean_oauth_stores() -> None:
    now = _oauth_time.time()
    for store in (_oauth_states, _oauth_pending):
        expired = [k for k, v in store.items() if (v if isinstance(v, float) else v.get("expires_at", 0)) < now]
        for k in expired:
            store.pop(k, None)


@app.post("/api/amazon-ads/oauth/start")
def amazon_ads_oauth_start() -> Any:
    """Generate an Amazon LWA authorization URL and return it.

    The frontend redirects the user to this URL in a new tab.
    """
    from urllib.parse import urlencode
    from amazon_ads.config import LWA_AUTHORIZE_URL, LWA_SCOPE

    client_id = _os.getenv("LWA_CLIENT_ID", "").strip()
    client_secret = _os.getenv("LWA_CLIENT_SECRET", "").strip()
    redirect_uri = _os.getenv("AMAZON_ADS_OAUTH_REDIRECT_URI", "").strip()

    if not client_id or not client_secret:
        return jsonify({"error": (
            "LWA_CLIENT_ID and LWA_CLIENT_SECRET must be set in your .env file "
            "before you can use the OAuth login flow."
        )}), 400

    if not redirect_uri:
        # Default to the current server's callback URL
        redirect_uri = request.host_url.rstrip("/") + "/api/amazon-ads/oauth/callback"

    state = _secrets.token_urlsafe(32)
    _oauth_states[state] = _oauth_time.time() + 300  # 5-minute window
    _clean_oauth_stores()

    auth_url = LWA_AUTHORIZE_URL + "?" + urlencode({
        "client_id": client_id,
        "scope": LWA_SCOPE,
        "response_type": "code",
        "redirect_uri": redirect_uri,
    ...
    return jsonify({"auth_url": auth_url, "state": state, "redirect_uri": redirect_uri})


@app.get("/api/amazon-ads/oauth/callback")
def amazon_ads_oauth_callback():
    """Amazon redirects here with ?code=…&state=… after user authorizes.

    Exchanges the code for a refresh token, discovers all Ad profiles, then
    renders oauth_callback.html so the user can name and save the account.
    """
    import requests as _req
    from amazon_ads.config import LWA_TOKEN_URL, AmazonAdsConfig
    from amazon_ads.auth import TokenManager
    from amazon_ads.client import AmazonAdsClient

    # --- handle authorization errors from Amazon ---
    amazon_error = request.args.get("error", "").strip()
    if amazon_error:
        desc = request.args.get("error_description", "")
        return render_template("oauth_callback.html", state="error",
            error_message=f"Amazon returned an error: {amazon_error}. {desc}".strip())
    ...
    if not code:
        return render_template("oauth_callback.html", state="error",
            error_message="No authorization code was received from Amazon. Try connecting again.")

    # --- CSRF state validation ---
    now = _oauth_time.time()
    if state not in _oauth_states or _oauth_states.get(state, 0) < now:
        return render_template("oauth_callback.html", state="error",
            error_message="The OAuth state token is missing or expired. Please start the connection again.")
    ...
    try:
        resp = _req.post(LWA_TOKEN_URL, data={
        ...
        if resp.status_code != 200:
            return render_template("oauth_callback.html", state="error",
                error_message=f"Token exchange failed (HTTP {resp.status_code}): {resp.text[:300]}")
        tokens = resp.json()
    except Exception as exc:
        return render_template("oauth_callback.html", state="error",
            error_message=f"Token exchange error: {exc}")
    ...
    if not refresh_token:
        return render_template("oauth_callback.html", state="error",
            error_message=(
                ...
                "Make sure offline_access is enabled and the LWA app scope is "
                "'advertising::campaign_management'."
            ))

    # --- Discover profiles across all three Amazon Ads regions ---
    temp_cfg = AmazonAdsConfig(
        client_id=client_id,
        client_secret=client_secret,
    ...
    temp_tm = TokenManager(config=temp_cfg)

    all_profiles: list[dict] = []
    profile_errors: list[str] = []
    seen_pids: set[str] = set()

    # One representative marketplace per region covers all profiles in that region
    for region_mp in ("US", "UK", "JP"):
        try:
            c = AmazonAdsClient(marketplace=region_mp, token_manager=temp_tm)
            r = c.get("/v2/profiles", require_profile=False)
            if r.status_code == 200:
                profs = r.json()
                if isinstance(profs, list):
                    for p in profs:
                        pid = str(p.get("profileId", "")).strip()
                        if not pid or pid in seen_pids:
                            continue
                        seen_pids.add(pid)
                        acc_info = p.get("accountInfo") or {}
                        all_profiles.append({
                            "profileId": pid,
                            "countryCode": (p.get("countryCode") or "").upper(),
                            "name": (acc_info.get("name") or p.get("name") or "").strip(),
                            "type": (acc_info.get("type") or "advertiser").lower(),
                            "currencyCode": (p.get("currencyCode") or "").upper(),
                            "timezone": p.get("timezone") or "",
                        })
            else:
                profile_errors.append(f"{region_mp}: HTTP {r.status_code}")
        except Exception as exc:
            profile_errors.append(f"{region_mp}: {str(exc)[:100]}")

    # --- Store pending auth data for the save step ---
    tmp_id = _secrets.token_urlsafe(24)
    _oauth_pending[tmp_id] = {
        "refresh_token": refresh_token,
        "profiles": all_profiles,
        "expires_at": _oauth_time.time() + 600,  # 10-minute window to save
    }
    _clean_oauth_stores()
...
@app.post("/api/amazon-ads/oauth/save")
def amazon_ads_oauth_save() -> Any:
    """Save a freshly-authorized Amazon account after the OAuth callback."""
    body = request.get_json(silent=True) or {}
    ...
    if not tmp_id or not label:
        return jsonify({"error": "tmp_id and label are required"}), 400

    pending = _oauth_pending.get(tmp_id)
    if not pending or pending.get("expires_at", 0) < _oauth_time.time():
        _oauth_pending.pop(tmp_id, None)
        return jsonify({"error": "Session expired or invalid. Please start the OAuth flow again."}), 400

    try:
        acct_id = bookdb.save_amazon_ads_account(
            label=label,
            lwa_refresh_token=pending["refresh_token"],
            env=env,
            client_id="",
            lwa_client_secret="",
            notes=notes,
    ...
    return jsonify({
        "ok": True,
        "id": acct_id,
        "profile_count": len(pending.get("profiles", [])),
    })


@app.get("/api/amazon-ads/accounts/<account_id>/test")
def test_amazon_ads_account(account_id: str) -> Any:
    """Test a specific account's refresh token by fetching its US profiles."""
    ...
    for mp in ("US", "UK", "CA", "AU"):
        try:
            ...
            if isinstance(profs, list):
                results[mp] = {"ok": True, "count": len(profs)}
                total_profiles += len(profs)
            else:
                results[mp] = {"ok": False, "error": str(profs)}
        except Exception as exc:
            results[mp] = {"ok": False, "error": str(exc)[:120]}
    connected = any(v.get("ok") for v in results.values())
    return jsonify({
        "connected": connected,
